...
Imported from the Guitar-Pedal-Plugin-Template harness and customised for Monarch of Tone. Static
SVG/HTML only (no chart JS libs, no CDN) — safe to open standalone or publish as an Artifact. Reads
the JSON comprehensive_report.py already wrote; does not re-render anything.

Note on "revision": captures.py repurposes the template's per-revision grouping as per-CLIPPING-
MODE (Boost/Overdrive/Distortion) — this project has one plugin revision, not several, so every
"rev" section below reads as a Boost-vs-OD-vs-Distortion breakdown instead, matching the per-mode
null-depth reporting style already used in CLAUDE.md.

Run from repo root (after `python3 analysis/comprehensive_report.py`):
  python3 analysis/dashboard_gen.py [--out analysis/reports/dashboard.html]
"""
import argparse
import json
import math
...
def build_fr_linecharts(captures, bands):
    """One small-multiple SVG line chart per clipping mode: FR shape delta vs log-frequency."""
    W, H = 520, 220
...
source: analysis/reports/comprehensive_data.json (regenerate with <code>python3 analysis/comprehensive_report.py</code>) &middot;
"revision" groups below are clipping mode (Boost/Overdrive/Distortion) &mdash; this project has one plugin revision, not several</div>

<h2>Per-mode headline</h2>
